...
public class MainFragment extends Fragment {
    ...
    private void initControls() {
        ...
        btn_sin.setOnClickListener(v -> {
            if (is2rnd) writeText("arcsin(", true, true);
            else writeText("sin(", true, true);
        });
        btn_cos.setOnClickListener(v -> {
            if (is2rnd) writeText("arccos(", true, true);
            else writeText("cos(", true, true);
        });
        btn_tan.setOnClickListener(v -> {
            if (is2rnd) writeText("arctan(", true, true);
            else writeText("tan(", true, true);
        });
        btn_pad_dot.setOnClickListener(v -> writeText(is2rnd ? "," : ".", false, false));

        root.findViewById(R.id.btn_pad_equal).setOnClickListener(v -> {
            vibratorVibrate();
            doCalc();
        });
    }
        ...
        if (onPadModeChangedListener != null) {
            switch (mode) {
                ...
                case PAD_MODE_SCIENCE:
                    onPadModeChangedListener.onPadModeChanged(text_mode_expand);
                    break;
                case PAD_MODE_PROGRAMMER:
                    onPadModeChangedListener.onPadModeChanged(text_mode_programmer);
                    break;
            }
        }
    }
